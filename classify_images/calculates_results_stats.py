...
def calculates_results_stats(results_dic):
    ...
    for key, value in results_dic.items():
        ...
        if value[3] == 1:
            n_dogs_img += 1
            # Check if the classifier label also indicates a dog
            if value[4] == 1:
                n_correct_dogs += 1
                # Check if the pet image and classifier label match
                if value[2] == 1:
                    n_match += 1
                    n_correct_breed += 1
        else:
            n_notdogs_img += 1
            # Check if the classifier label also indicates a non-dog
            if value[4] == 0:
                n_correct_notdogs += 1
            # Check if the pet image and classifier label match (for non-dogs)
            if value[2] == 1:
                n_match += 1
